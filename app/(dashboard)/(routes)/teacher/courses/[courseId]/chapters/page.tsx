...
const CourseIdPage = async ({ params } :  { params: { courseId: string } }) => {
  ...
  return (
    <>
      {!course.isPublished && (
        <Banner label="This course is invisible to students because it is not published" />
      )}
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex-col gap-y-2">
            <h1 className="text-2xl font-medium">Course setup</h1>
            <span className="text-sm text-slate">Complete all fields {completionText}</span>
          </div>
          <Actions
            disabled={!isComplete}
